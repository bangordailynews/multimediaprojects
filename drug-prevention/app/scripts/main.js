...
function sendGoogleAnalyticsEvent( eventAction, eventLabel, eventValue ) {
    ...
    ga('send', {
        ...
        'eventCategory': 'drug prevention', // Required.
    ...
    var videos = {};
       $.getJSON(
        ...
        function(mainChapters) {
            ...
            /** Determine current page chapter */
            $.each(mainChapters, function(index, value)
            {
                /** Append Chapters to nav */
                console.log("index = " + index)
                if( index == 0 || value == "intro" ) {
                    $('#top-nav-container').append("<li class='nav_intro_logo'><a href=?" + value.slug + "><img src='images/nav_intro_logo.png' height='50px'><span>" + value.nav_name.regular + "</span></a></li>");
                } else {
                    $('#top-nav-container').append("<li><a href=?" + value.slug + ">" + value.nav_name.regular + "</a></li>");
                }

                /** Set machine name */
                mainChapters[index].machineName = value.slug.replace(/ /g, '-').toLowerCase();

                if( location.href.indexOf( value.slug ) > 0 ) {
                    displayChapter = value;
                    displayChapter.count = index;

                    // Highlight the option in the nav
                    $( '#top-nav-container li' ).removeClass('active');
                    $( 'a[href="?' + value.slug + '"]' ).parent().addClass('active');
                    //$( '#top-nav-container li a[href="?' + value.slug + '"]' ).text( value.nav_name.regular )
                } 
            });//each mainChapters
            ...
            console.log('Get chapter data: /data/' + displayChapter.machineName + '.json');
